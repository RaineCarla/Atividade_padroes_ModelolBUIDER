...
public class PedidoBuilder {
    ...
    public Pedido build() {
            ...
            if (itens.size() == 0)
                throw new IllegalStateException("Pedido precisa ter pelo menos um item");


            if (!"DELIVERY".equals(entrega) && !"RETIRADA".equals(entrega))
                throw new IllegalStateException("Tipo de entrega inválido");

            if ("HAMBURGUER".equals(tipo)) {

                if (tipoPao == null)
                    throw new IllegalStateException("Hamburguer precisa de pão");
            ...
            if (!"DINHEIRO".equals(pagamento) && trocoPara != null)
                throw new IllegalStateException("Troco só permitido para dinheiro");
